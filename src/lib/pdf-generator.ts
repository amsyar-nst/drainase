...
export const generateDrainaseReportPDF = async (
  ...
  if (reportType === "harian") {
    ...
    `;
  } else { // reportType === "bulanan" (or any other type not explicitly defined)
    htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="header">
          ...
          <div class="subtitle">UPT OPERASIONAL PEMELIHARAAN JALAN DAN DRAINASE MEDAN KOTA</div>
        </div>
          ...
          <tbody>
            ${kegiatansWithImages.map((kegiatan, index) => `
              <tr>
                ...
                <td>${kegiatan.namaJalan}, ${kegiatan.kelurahan}, ${kegiatan.kecamatan}</td>
                <td class="center">${kegiatan.jenisSaluran || '-'}</td>
                <td class="center">${kegiatan.jenisSedimen || '-'}</td>
                <td>${kegiatan.aktifitasPenanganan}</td>
                <td class="center">${kegiatan.panjangPenanganan || '-'}</td>
